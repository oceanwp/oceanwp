 ...
 * 
 * Bottom template for metas without icons
 *
...
// Get meta sections.
$sections = ocean_layout_bottom_meta();

// Don't display modified date if the same as the published date.
$ocean_date_onoff = false;
$ocean_date_onoff = apply_filters( 'ocean_bottom_meta_modified_date_state', $ocean_date_onoff );
$display_mod_date = ( false === $ocean_date_onoff || ( true === $ocean_date_onoff && ( get_the_date() != get_the_modified_date() ) ) ) ? true : false;

do_action( 'ocean_before_bottom_layout_meta' );
?>

<ul class="bottom-entry-meta-2 <?php echo ocean_layout_bottom_meta_separator_class(); ?>">
...
<?php do_action( 'ocean_after_bottom_layout_meta' ); ?>
