 ...
 */
if ( get_theme_mod( 'ocean_disable_wp_emojis', false ) ) {
	add_action( 'init', 'oceanwp_disable_emojis' );

	/**
	 * Disable the WP emoji's function.
	 *
	 * @return void
	 */
	function oceanwp_disable_emojis() {
		...
		add_filter( 'tiny_mce_plugins', 'oceanwp_disable_emojis_tinymce' );
		add_filter( 'wp_resource_hints', 'oceanwp_disable_emojis_remove_dns_prefetch', 10, 2 );
	}
	/**
	 ...
	 */
	function oceanwp_disable_emojis_tinymce( $plugins ) {
		if ( is_array( $plugins ) ) {
	 ...
	 * @return array Difference betwen the two arrays.
	 */
	function oceanwp_disable_emojis_remove_dns_prefetch( $urls, $relation_type ) {
