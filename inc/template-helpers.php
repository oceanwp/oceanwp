...
if ( ! function_exists( 'ocean_get_post_author' ) ) {
	...
	function ocean_get_post_author( $args = array(), $echo = true ) {
		...
		$author = sprintf(
			...
			'<a href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '" rel="author" aria-label="' . esc_attr( $aria_label ) . '">' . esc_html( get_the_author() ) . '</a>'
		);
...
/**
 * Returns top meta sections for blog post entries
 *
 * @since 3.1.0
 */
if ( ! function_exists( 'ocean_layout_top_meta' ) ) {

	function ocean_layout_top_meta() {

		// Default sections.
		$sections = array( 'author', 'date', 'categories', 'comments', 'mod-date', 'reading-time', 'tags' );

		// Get sections from Customizer.
		$sections = get_theme_mod( 'ocean_top_entry_meta', $sections );
		...
		// Apply filters for easy modification.
		$sections = apply_filters( 'oceanwp_layout_top_meta', $sections );
...
/**
 * Return new post entries meta style from the Customizer
 * 
 * @since 3.1.0
 */
if ( ! function_exists( 'ocean_layout_top_meta_style' ) ) {

	function ocean_layout_top_meta_style() {

		$style = get_theme_mod( 'oceanwp_top_entry_meta_style' );

		$style = $style ? $style : 'betm_style_2';

		return apply_filters( 'oceanwp_layout_top_meta_style', $style );
	}
}

/**
 * Return new post entries top meta template
 * 
 * @since 3.1.0
 */
if ( ! function_exists( 'ocean_layout_top_meta_template' ) ) {

	function ocean_layout_top_meta_template() {

		$template_style = ocean_layout_top_meta_style();
		$template_path = '';

		if ( 'betm_style_2' === $template_style ) {
			$template_path = 'partials/entry/metas/meta-top-2';
		} elseif ( 'betm_style_3' === $template_style ) {
			$template_path = 'partials/entry/metas/meta-top-3';
		}

		return apply_filters( 'oceanwp_layout_top_meta_template', $template_path );
	}
}

/**
 * Return new post entries top meta template part
 * 
 * @since 3.1.0
 */
if ( ! function_exists( 'ocean_layout_top_meta_template_part' ) ) {

	function ocean_layout_top_meta_template_part() {

		$template_part = ocean_layout_top_meta_template();

		get_template_part( $template_part );
	}

	add_action( 'ocean_layout_top_meta', 'ocean_layout_top_meta_template_part' );
}

/**
 * Return blog post entries top meta separator
 * 
 * @return string
 * @since 3.1.0
 */
if ( ! function_exists( 'ocean_layout_top_meta_separator_class' ) ) {

	function ocean_layout_top_meta_separator_class() {

		$separator = ocean_layout_top_meta_separator_style();
		...
		return apply_filters( 'oceanwp_layout_top_meta_separator_class', $class_style );
	}
}

/**
 * Return blog entries top meta separator
 * 
 * @since 3.1.0
 */
if ( ! function_exists( 'ocean_layout_top_meta_separator_style' ) ) {

	function ocean_layout_top_meta_separator_style() {

		$separator = get_theme_mod( 'oceanwp_top_entry_meta_style_separator', 'stylish' );

		$separator = $separator ? $separator : 'stylish';

		return apply_filters( 'oceanwp_layout_top_meta_separator_style', $separator );

	}
}

/**
 * Returns bottom meta sections for blog post entries
 *
 * @since 3.1.0
 */
if ( ! function_exists( 'ocean_layout_bottom_meta' ) ) {

	function ocean_layout_bottom_meta() {

		// Default sections.
		$sections = array( 'author', 'date', 'categories', 'comments', 'mod-date', 'reading-time', 'tags' );

		// Get sections from Customizer.
		$sections = get_theme_mod( 'ocean_bottom_entry_meta', $sections );
		...
		// Apply filters for easy modification.
		$sections = apply_filters( 'oceanwp_layout_bottom_meta', $sections );
 ...
 * @since 3.1.0
 */
if ( ! function_exists( 'ocean_layout_bottom_meta_style' ) ) {

	function ocean_layout_bottom_meta_style() {

		$style = get_theme_mod( 'oceanwp_bottom_entry_meta_style' );

		$style = $style ? $style : 'bebm_style_2';

		return apply_filters( 'oceanwp_layout_bottom_meta_style', $style );
	}
}

/**
 * Return new post entries bottom meta template
 * 
 * @since 3.1.0
 */
if ( ! function_exists( 'ocean_layout_bottom_meta_template' ) ) {

	function ocean_layout_bottom_meta_template() {

		$template_style = ocean_layout_bottom_meta_style();
		$template_path = '';

		if ( 'bebm_style_2' === $template_style ) {
			$template_path = 'partials/entry/metas/meta-bottom-2';
		} elseif ( 'bebm_style_3' === $template_style ) {
			$template_path = 'partials/entry/metas/meta-bottom-3';
		}

		return apply_filters( 'oceanwp_layout_bottom_meta_template', $template_path );
	}
}

/**
 * Return new post entries bottom meta template part
 * 
 * @since 3.1.0
 */
if ( ! function_exists( 'ocean_layout_bottom_meta_template_part' ) ) {

	function ocean_layout_bottom_meta_template_part() {

		$template_part = ocean_layout_bottom_meta_template();

		get_template_part( $template_part );
	}

	add_action( 'ocean_layout_bottom_meta', 'ocean_layout_bottom_meta_template_part' );
}

/**
 * Return blog post entries bottom meta separator
 * 
 * @return string
 * @since 3.1.0
 */
if ( ! function_exists( 'ocean_layout_bottom_meta_separator_class' ) ) {

	function ocean_layout_bottom_meta_separator_class() {

		$separator = ocean_layout_bottom_meta_separator_style();
		...
		return apply_filters( 'oceanwp_layout_bottom_meta_separator_class', $class_style );
	}
}

/**
 * Return blog entries bottom meta separator
 * 
 * @since 3.1.0
 */
if ( ! function_exists( 'ocean_layout_bottom_meta_separator_style' ) ) {

	function ocean_layout_bottom_meta_separator_style() {

		$separator = get_theme_mod( 'oceanwp_bottom_entry_meta_style_separator', 'stylish' );

		$separator = $separator ? $separator : 'stylish';

		return apply_filters( 'oceanwp_layout_bottom_meta_separator_style', $separator );
...
if ( ! function_exists( 'oceanwp_theme_blog_meta_separator' ) ) {
	...
	function oceanwp_theme_blog_meta_separator() {
		...
		if ( 'default' === $meta_style ) {
			$class = 'obem-default';
		} elseif ( 'modern' === $meta_style ) {
			$class = 'obem-modern';
		} elseif ( 'stylish' === $meta_style ) {
			$class = 'obem-stylish';
		}

		return apply_filters( 'oceanwp_theme_blog_meta_separator', $class );
	}
}

/**
 * Return Blog Entries Layout Meta Style
 * 
 * @since 3.1.0
 */
if ( ! function_exists( 'ocean_blog_entry_layout_meta_style' ) ) {

	function ocean_blog_entry_layout_meta_style() {

		$style = get_theme_mod( 'ocean_blog_entry_meta_style', 'default' );

		$style = $style ? $style : 'default';

		return apply_filters( 'oceanwp_blog_entry_layout_meta_style', $style );

	}
}

/**
 * Get Blog Entry layout meta template
 * 
 * @since 3.1.0
 */
if ( ! function_exists( 'ocean_blog_entry_meta_style_template' ) ) {

	function ocean_blog_entry_meta_style_template() {

		$style = ocean_blog_entry_layout_meta_style();

		$template_path = '';

		if ( 'default' === $style ) {
			$template_path = 'partials/entry/meta';
		} else {
			$template_path = 'partials/entry/metas/meta-2';
		}

		return apply_filters( 'oceanwp_blog_entry_meta_style_template', $template_path );
	}
}
